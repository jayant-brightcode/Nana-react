...
const WorkDashboardScreen = () => {
        ...
        try {
           ...
           // const urlWithParams = `${apiUrl}?${new URLSearchParams(queryParams)}`;
            ...
            if (response.ok) {
                ...
                console.log("FfFffF",data.monthly_dates)
    ...
    const render_work_history = ({ item }) => (
        ...
        <View style={{ margin: '4%' }}>
            ...
            <Text style={{ fontWeight: 'medium', color: Colors.textcolor, fontSize: 15, marginTop: 20 }}>From : {item.user.name} </Text>
            <Text style={{ fontWeight: 'medium', color: Colors.textcolor, fontSize: 15, marginTop: 20 }}>To : {item.to_user.name} </Text>


            <Text style={{ fontWeight: 'medium', color: Colors.textcolor, fontSize: 15, marginTop: 15 }}>Profession Skill : {item.skills.map(skill => skill.name).join(', ')}</Text>

            <Text style={{ fontWeight: 'medium', color: Colors.textcolor, fontSize: 15, marginTop: 20 }}>Profession Name : {item.to_user.profession_name} </Text>

            <Text style={{ fontWeight: 'medium', color: Colors.red, fontSize: 15, marginTop: 15 }}>Service Charge - Rs.{item.job_request.amount_set}</Text>

            <Text style={{ fontWeight: 'medium', color: Colors.red, fontSize: 15, marginTop: 15 }}>Platform amount paid by  {item.user.name} - Rs.{item.payment_amount.toString()}</Text>
...
<View style={{ margin: '4%' }}>
          ...
          <Text style={{ fontWeight: 'medium', color: Colors.textcolor, fontSize: 15, marginTop: 15 }}>Profession Name -  {item.to_user.profession_name}</Text>

                    <Text style={{ fontWeight: 'medium', color: Colors.red, fontSize: 15, marginTop: 15 }}>Monthly Salary - Rs.{item.job_request.amount_set}</Text>
    ...
    const render_monthly_dates = ({ item }) => (
       ...
       <TouchableOpacity onPress={()=>{
        ...
        <View style={{backgroundColor:Colors.gray,padding:7,marginTop:5}}>


             <Text style={{ fontWeight: 'bold', color: Colors.orange, fontSize: 12, marginTop: 4 }}>From  : {item.from_user_id.name} </Text>

             <Text style={{ fontWeight: 'bold', color: Colors.orange, fontSize: 12, marginTop: 4 }}>To  : {item.to_user_id.name} </Text>
            ...
            <View>
                <View>
                    <TouchableOpacity onPress={openDialog}>
                        <Text style={{margin:10,borderRadius:10,borderWidth:1,overflow:'hidden',borderColor:Colors.orange,padding:10,color:Colors.orange,textAlign:'center'}}>Select Montly Job</Text>
                    </TouchableOpacity>
                </View>
        ...
        <Modal visible={isDialogVisible} animationType="slide" transparent>
                <View style={styles.modalContainer}>
                    <View style={styles.dialogContainer}>
                        <Text style={{ color: Colors.black, alignSelf: 'center', marginBottom: 20, fontWeight: 'bold' }}>Select Monthly Job to See work details</Text>
