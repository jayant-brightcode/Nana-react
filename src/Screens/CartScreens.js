...
import {RefreshControl, View, Text, StyleSheet, Image, FlatList, TouchableOpacity,TextInput,TouchableWithoutFeedback,Modal,ActivityIndicator,ScrollView, SafeAreaView } from 'react-native'
...
const CartScreens = () => {
   ...
   // const { user } = route.params
    ...
    const [cart_details,set_cart_details] = useState({})
    ...
    const renderCart = ({ item }) => { 
     ...
     const skillsString = item.to_user_id.skills.map(skill => skill.name).join(', ');
        ...
        <TouchableOpacity onPress={() => {
            ...
            //    set_user(item.to_user_id)
            //    set_cart_details(item)
...
<Modal visible={isUserTypeDialogVisible} animationType="slide" transparent>
                   ...
                   <Text style={{marginStart:10,fontWeight:'bold',alignSelf:'center',color:Colors.orange}}>Important Note</Text>
                      <Text style={{marginStart:10,marginEnd:10,marginTop:10,alignSelf:'center'}}>** For this feature you need to update your category type to Emoloyer, you can change this anytime from your profile page **</Text>
