...
import { View, Text, StyleSheet, Image, FlatList, TouchableOpacity,TextInput,TouchableWithoutFeedback,Modal,ActivityIndicator,ScrollView, SafeAreaView } from 'react-native'
...
const PolicyScreen = () => {
    ...
    const navigation = useNavigation()
     ...
     const [policy,set_policy] = useState('')
     const [loading, setLoading] = useState(false);
     const htmlContent = `<html><head><style>body { font-size: 44px; }</style></head><body>${policy}</body></html>`;
    ...
    const get_policy = async () => {
        ...
        try {
            ...
            let apiUrl = Remote.BASE_URL + "get_privacy"
            ...
            if (response.ok) {
                ...
                set_policy(data.privacy_policy.privacy_policy);
           ...
           <View style={styles.topbar}>
            <View style={{ flexDirection: 'row', marginTop: Platform.OS=="android"?10:0, padding: 10, alignItems: 'center' }}>
                ...
                <View style={{ marginStart: 15, flex: 1 }}>
                    <Text style={{ color: Colors.white, fontWeight: 'bold', fontSize: 20 }}>Privacy Policy</Text>
...
<ScrollView style={{ flex: 1, width: '95%', alignSelf: 'center' ,height:'100%',backgroundColor:Colors.white}}>
            <WebView
